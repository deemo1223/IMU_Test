...
#ifndef LIBBIOMIMETICS_ORIENTATION_TOOLS_MODIFY_H
#define LIBBIOMIMETICS_ORIENTATION_TOOLS_MODIFY_H
...
Mat3<typename T::Scalar> rpyToRotMat(const Eigen::MatrixBase<T>& v) {
  ...
  Mat3<typename T::Scalar> m = coordinateRotation(CoordinateAxis::Z, v[2]) *
                               coordinateRotation(CoordinateAxis::Y, v[1]) *
                               coordinateRotation(CoordinateAxis::X, v[0]);
  return m;
}
 ...
 *
 * For a quaternion q = [w, x, y, z], the conjugate is defined as:
 *   q* = [w, -x, -y, -z]
 *
 * @tparam T Floating point type.
 * @param q A 4x1 quaternion (order: [w, x, y, z]).
 * @return A 4x1 quaternion representing the conjugate of q.
 */
template <typename Derived>
typename Derived::PlainObject quatConjugate(const Eigen::MatrixBase<Derived>& q) {
  static_assert(Derived::ColsAtCompileTime == 1 && Derived::RowsAtCompileTime == 4,
                "Input must be a 4x1 quaternion [w, x, y, z]");
  typename Derived::PlainObject q_conj;
  q_conj[0] = q[0];
  q_conj[1] = -q[1];
  q_conj[2] = -q[2];
  q_conj[3] = -q[3];
